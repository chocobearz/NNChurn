...
selected_columns <- colnames(cd)[-20]

for( i in c(1:length(selected_columns)) ){
    print(
        ggplot(cd, aes_string(x = selected_columns[i], 
                              y = "churn")) + 
        geom_point() +
        xlab("regressor")
    )
...
# I don't think cd has paymentmethodA and paymentmethodB here
